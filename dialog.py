...
from qgis.PyQt.QtGui import (
    QFontDatabase, QPixmap, QPainter, QColor, QFont
)
...
class QRVIZDialog(QDialog):
    ...
    def _build_single_map_tab(self):
        ...
        self.lbl_r = QLabel("R:")
        self.sp_r = QSpinBox()
        self.sp_r.setRange(1, 100)
        self.sp_r.setValue(1)

        self.lbl_g = QLabel("G:")
        self.sp_g = QSpinBox()
        self.sp_g.setRange(1, 100)
        self.sp_g.setValue(2)

        self.lbl_b = QLabel("B:")
        self.sp_b = QSpinBox()
        self.sp_b.setRange(1, 100)
        self.sp_b.setValue(3)

        for w in [self.lbl_r, self.sp_r, self.lbl_g, self.sp_g, self.lbl_b, self.sp_b]:
            w.setVisible(False)

        band_grid.addWidget(self.lbl_r, 1, 0)
        band_grid.addWidget(self.sp_r, 1, 1)
        band_grid.addWidget(self.lbl_g, 1, 2)
        band_grid.addWidget(self.sp_g, 1, 3)
        band_grid.addWidget(self.lbl_b, 2, 0)
        band_grid.addWidget(self.sp_b, 2, 1)
        ...
        g2.setSpacing(10)
        g2.setContentsMargins(8, 10, 8, 8)
        ...
        self.lbl_pmin = QLabel("Pmin (%):")
        self.sp_pmin = QDoubleSpinBox()
        self.sp_pmin.setRange(0, 49)
        self.sp_pmin.setValue(2)
        self.sp_pmin.setSingleStep(0.5)

        self.lbl_pmax = QLabel("Pmax (%):")
        self.sp_pmax = QDoubleSpinBox()
        self.sp_pmax.setRange(51, 100)
        self.sp_pmax.setValue(98)
        self.sp_pmax.setSingleStep(0.5)

        g2.addWidget(self.lbl_pmin, 1, 0)
        g2.addWidget(self.sp_pmin, 1, 1)
        g2.addWidget(self.lbl_pmax, 1, 2)
        g2.addWidget(self.sp_pmax, 1, 3)

        self.lbl_vmin = QLabel("vmin:")
        self.le_vmin = QLineEdit("0")
        self.lbl_vmax = QLabel("vmax:")
        self.le_vmax = QLineEdit("1")

        g2.addWidget(self.lbl_vmin, 2, 0)
        g2.addWidget(self.le_vmin, 2, 1)
        g2.addWidget(self.lbl_vmax, 2, 2)
        g2.addWidget(self.le_vmax, 2, 3)

        g2.addWidget(QLabel("Colormap:"), 3, 0)
        cmap_lay = QHBoxLayout()
        cmap_lay.setContentsMargins(0, 0, 0, 0)
        cmap_lay.setSpacing(4)

        self.btn_cmap_prev = QPushButton("◀")
        self.btn_cmap_prev.setObjectName("btn_arrow")
        self.btn_cmap_prev.setFixedSize(26, 26)
        self.btn_cmap_prev.clicked.connect(self._cmap_prev)
        ...
        self.btn_cmap_next = QPushButton("▶")
        self.btn_cmap_next.setObjectName("btn_arrow")
        self.btn_cmap_next.setFixedSize(26, 26)
        self.btn_cmap_next.clicked.connect(self._cmap_next)
        ...
        disc_outer.setContentsMargins(8, 8, 8, 8)
        disc_outer.setSpacing(6)
        ...
        self.sp_global_decimals.setRange(0, 6)
        self.sp_global_decimals.setValue(2)
        self.sp_global_decimals.setFixedWidth(50)
        dec_row.addWidget(self.sp_global_decimals)
        btn_apply_dec = QPushButton("Apply")
        btn_apply_dec.setFixedHeight(26)
        btn_apply_dec.clicked.connect(self._apply_global_decimals)
        dec_row.addWidget(btn_apply_dec)
        dec_row.addStretch()
        ...
        self.le_nodata_hex.setFixedWidth(90)
        self.le_nodata_hex.setReadOnly(True)
        nd_row.addWidget(self.btn_nodata_color)
        nd_row.addWidget(self.le_nodata_hex)
        nd_row.addStretch()
        ...
        g_exp.setSpacing(8)
        g_exp.setContentsMargins(12, 18, 12, 12)
        ...
        centre_lay.setContentsMargins(4, 0, 4, 0)
        ...
        self.sp_title_size.setRange(6, 40)
        self.sp_title_size.setValue(14)
        ...
        gmc.setSpacing(10)
        gmc.setContentsMargins(12, 18, 12, 12)

        gmc.addWidget(QLabel("Pos X:"), 0, 0)
        self.sp_map_x = QDoubleSpinBox()
        self.sp_map_x.setRange(0, 1)
        self.sp_map_x.setSingleStep(0.01)
        self.sp_map_x.setValue(0.08)
        gmc.addWidget(self.sp_map_x, 0, 1)

        gmc.addWidget(QLabel("Pos Y:"), 0, 2)
        self.sp_map_y = QDoubleSpinBox()
        self.sp_map_y.setRange(0, 1)
        self.sp_map_y.setSingleStep(0.01)
        self.sp_map_y.setValue(0.12)
        gmc.addWidget(self.sp_map_y, 0, 3)

        gmc.addWidget(QLabel("Width:"), 1, 0)
        self.sp_map_w = QDoubleSpinBox()
        self.sp_map_w.setRange(0.05, 1)
        self.sp_map_w.setSingleStep(0.01)
        self.sp_map_w.setValue(0.85)
        gmc.addWidget(self.sp_map_w, 1, 1)

        gmc.addWidget(QLabel("Height:"), 1, 2)
        self.sp_map_h = QDoubleSpinBox()
        self.sp_map_h.setRange(0.05, 1)
        self.sp_map_h.setSingleStep(0.01)
        self.sp_map_h.setValue(0.80)
        ...
        self.sp_coord_size = QSpinBox()
        self.sp_coord_size.setRange(4, 30)
        self.sp_coord_size.setValue(9)
        gmc.addWidget(self.sp_coord_size, 3, 1)

        gmc.addWidget(QLabel("Coord Decimals:"), 3, 2)
        self.sp_coord_decimals = QSpinBox()
        self.sp_coord_decimals.setRange(0, 8)
        self.sp_coord_decimals.setValue(4)
        ...
        self.sp_xlabel_rotation.setRange(0, 360)
        self.sp_xlabel_rotation.setValue(0)
        ...
        self.sp_ylabel_rotation.setRange(0, 360)
        self.sp_ylabel_rotation.setValue(90)
        ...
        self.sp_xtick_count = QSpinBox()
        self.sp_xtick_count.setRange(2, 20)
        self.sp_xtick_count.setValue(5)
        gmc.addWidget(self.sp_xtick_count, 5, 1)

        gmc.addWidget(QLabel("Y Tick Count:"), 5, 2)
        self.sp_ytick_count = QSpinBox()
        self.sp_ytick_count.setRange(2, 20)
        self.sp_ytick_count.setValue(5)
        ...
        gcb.setSpacing(10)
        gcb.setContentsMargins(12, 18, 12, 12)
        ...
        self.sp_leg_x = QDoubleSpinBox()
        self.sp_leg_x.setRange(0, 1)
        self.sp_leg_x.setSingleStep(0.01)
        self.sp_leg_x.setValue(0.20)
        gcb.addWidget(self.sp_leg_x, 2, 1)

        gcb.addWidget(QLabel("Pos Y:"), 2, 2)
        self.sp_leg_y = QDoubleSpinBox()
        self.sp_leg_y.setRange(0, 1)
        self.sp_leg_y.setSingleStep(0.01)
        self.sp_leg_y.setValue(0.05)
        gcb.addWidget(self.sp_leg_y, 2, 3)

        gcb.addWidget(QLabel("Length:"), 3, 0)
        self.sp_leg_w = QDoubleSpinBox()
        self.sp_leg_w.setRange(0.01, 1)
        self.sp_leg_w.setSingleStep(0.01)
        self.sp_leg_w.setValue(0.60)
        gcb.addWidget(self.sp_leg_w, 3, 1)

        gcb.addWidget(QLabel("Thickness:"), 3, 2)
        self.sp_leg_h = QDoubleSpinBox()
        self.sp_leg_h.setRange(0.01, 1)
        self.sp_leg_h.setSingleStep(0.01)
        self.sp_leg_h.setValue(0.03)
        gcb.addWidget(self.sp_leg_h, 3, 3)

        gcb.addWidget(QLabel("Label Size:"), 4, 0)
        self.sp_leg_lbl_size = QSpinBox()
        self.sp_leg_lbl_size.setRange(6, 30)
        self.sp_leg_lbl_size.setValue(10)
        gcb.addWidget(self.sp_leg_lbl_size, 4, 1)

        gcb.addWidget(QLabel("Tick Size:"), 4, 2)
        self.sp_leg_tick_size = QSpinBox()
        self.sp_leg_tick_size.setRange(6, 30)
        self.sp_leg_tick_size.setValue(9)
        gcb.addWidget(self.sp_leg_tick_size, 4, 3)

        self.chk_ticks = QCheckBox("Dynamic Ticks")
        self.chk_ticks.setChecked(False)
        gcb.addWidget(self.chk_ticks, 5, 0, 1, 2)

        gcb.addWidget(QLabel("Tick Count:"), 5, 2)
        self.sp_tick_count = QSpinBox()
        self.sp_tick_count.setRange(2, 30)
        self.sp_tick_count.setValue(5)
        gcb.addWidget(self.sp_tick_count, 5, 3)

        gcb.addWidget(QLabel("Label Pad:"), 6, 0)
        self.sp_leg_pad_label = QSpinBox()
        self.sp_leg_pad_label.setRange(-50, 100)
        self.sp_leg_pad_label.setValue(5)
        gcb.addWidget(self.sp_leg_pad_label, 6, 1)

        gcb.addWidget(QLabel("Tick Pad:"), 6, 2)
        self.sp_leg_pad_tick = QSpinBox()
        self.sp_leg_pad_tick.setRange(-50, 100)
        self.sp_leg_pad_tick.setValue(2)
        gcb.addWidget(self.sp_leg_pad_tick, 6, 3)

        gcb.addWidget(QLabel("Tick Decimals:"), 7, 0)
        self.sp_cbar_decimals = QSpinBox()
        self.sp_cbar_decimals.setRange(0, 8)
        self.sp_cbar_decimals.setValue(4)
    ...
    def _build_layout_series_tab(self):
        ...
        self.sp_layout_cols = QSpinBox()
        self.sp_layout_cols.setRange(1, 6)
        self.sp_layout_cols.setValue(2)
        cfg_lay.addWidget(self.sp_layout_cols)

        cfg_lay.addWidget(QLabel("Rows:"))
        self.sp_layout_rows = QSpinBox()
        self.sp_layout_rows.setRange(1, 6)
        self.sp_layout_rows.setValue(2)
        ...
        self.sp_hspace.setRange(0, 1)
        self.sp_hspace.setValue(0.25)
        ...
        self.sp_wspace.setRange(0, 1)
        self.sp_wspace.setValue(0.25)
        self.sp_wspace.setSingleStep(0.05)
        cfg_lay.addWidget(self.sp_wspace)

        cfg_lay.addWidget(QLabel("Fig size (in):"))
        self.sp_fig_w = QSpinBox()
        self.sp_fig_w.setRange(6, 40)
        self.sp_fig_w.setValue(16)
        cfg_lay.addWidget(self.sp_fig_w)
        cfg_lay.addWidget(QLabel("×"))
        self.sp_fig_h = QSpinBox()
        self.sp_fig_h.setRange(4, 30)
        self.sp_fig_h.setValue(10)
        ...
        self._slot_layout.setContentsMargins(4, 4, 4, 4)
    ...
    def _auto_position_legend(self, orient):
        ...
        if orient == "vertical":
            self.sp_leg_x.setValue(0.88)
            self.sp_leg_y.setValue(0.20)
            self.sp_leg_w.setValue(0.03)
            self.sp_leg_h.setValue(0.60)
        else:
            self.sp_leg_x.setValue(0.20)
            self.sp_leg_y.setValue(0.05)
            self.sp_leg_w.setValue(0.60)
    ...
    def _toggle_band_mode(self):
        ...
        self.lbl_band.setVisible(single)
        self.sp_band.setVisible(single)
        self.btn_cmap_prev.setEnabled(single)
        self.btn_cmap_next.setEnabled(single)
    ...
    def _toggle_stretch_opts(self):
        ...
        pct = "Percentile" in mode
        manual = "Manual" in mode
        self.lbl_pmin.setVisible(pct)
        self.sp_pmin.setVisible(pct)
        self.lbl_pmax.setVisible(pct)
        self.sp_pmax.setVisible(pct)
        self.lbl_vmin.setVisible(manual)
        self.le_vmin.setVisible(manual)
        self.lbl_vmax.setVisible(manual)
        self.le_vmax.setVisible(manual)
    ...
    def _read_and_render(self):
        ...
        try:
            if self.rb_single.isChecked():
                ...
                self._cached_rgb = (r, g, b)
                self._cached_ext = ext
                self._cached_arr = None
            self._trigger_live_update()
    ...
    def _render_discrete(self, ax, ax_bg, txt_col, font_fam, plot_ext):
        ...
        if self.chk_legend.isChecked() and self.chk_disc_legend.isChecked():
            ...
            for text in leg.get_texts():
                text.set_color(txt_col)
                text.set_fontfamily(font_fam)
